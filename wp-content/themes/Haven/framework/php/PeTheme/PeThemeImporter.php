...
class PeThemeImporter extends WP_Import {
	...
	public function backfill_attachment_urls() {
		...
		$attachment = wp_get_attachment_url($wpdb->get_var("SELECT ID FROM $pt WHERE $pt.post_type = 'attachment' LIMIT 1"));
		...
		$placeholder = 's:14:"--ATTACHMENT--"';
		$replace = sprintf('s:%d:"%s"',strlen($attachment),$attachment);
	...
	public function updateStats($message = "",$done = false) {
		...
		} else if ( ! empty( $stats["progress"] ) || ($stats["progress"] < $stats["total"])) {
